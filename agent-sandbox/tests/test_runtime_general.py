...
def test_run_python_code(runtime: SandboxRuntime) -> None:
    result = runtime.run_python_code("import sys; print(sys.argv[1].upper())", args=["job-agent"])
    assert result.ok
    assert result.stdout.strip() == "JOB-AGENT"
...
def test_cwd_and_env_are_propagated(runtime: SandboxRuntime, tmp_path: Path) -> None:
    result = runtime.run_python_code(
        "import os; print(os.getcwd()); print(os.environ['JOB_AGENT_TEST_ENV'])",
        cwd=tmp_path,
        env={"JOB_AGENT_TEST_ENV": "sandbox-env"},
    )
