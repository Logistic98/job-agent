...
- `code` 必须具备明确语义，错误码集中维护，禁止前后端各自约定。
...
- 每次让 Claude 生成代码前，先用自然语言把目标、边界、约束、禁止事项说清楚。
