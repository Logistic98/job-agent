...
const FOLDERS_KEY = 'job-agent.resume-folders'
const MAX_TAGS = 6
...
function openInWriter(item) {
  ...
  localStorage.setItem('job-agent.resume-writer.markdown', content)
  localStorage.setItem('job-agent.resume-writer.opened-resume-id', item.resumeId || '')
