...
def test_postgres_store_derives_dsn_from_spring_datasource(monkeypatch):
    ...
    monkeypatch.setenv("SPRING_DATASOURCE_URL", "jdbc:postgresql://127.0.0.1:5432/job_agent")
    monkeypatch.setenv("SPRING_DATASOURCE_USERNAME", "job_agent")
    monkeypatch.setenv("SPRING_DATASOURCE_PASSWORD", "")

    store = PostgresMemoryStore()

    assert store.dsn == "postgresql://job_agent@127.0.0.1:5432/job_agent"
