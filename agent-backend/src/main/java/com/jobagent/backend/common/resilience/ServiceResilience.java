package com.jobagent.backend.common.resilience;
...
/**
 * Dependency-free resilience layer for outbound calls to the Python agent services.
 * Provides bounded retry with backoff and a per-service circuit breaker so a single
 * unavailable downstream cannot stall every request on the configured read timeout.
 */
@Component
public class ServiceResilience {
    ...
    private final ConcurrentHashMap<String, Circuit> circuits = new ConcurrentHashMap<String, Circuit>();
    ...
    /**
     * Execute {@code action}, returning {@code fallback} when the circuit is open or all attempts fail.
     * Only retry idempotent operations; pass {@code retryable=false} for non-idempotent writes/runs.
     */
    ...
    public void recordFailure(String service) {
        ...
        if (failures >= Math.max(1, properties.getCircuitFailureThreshold())) {
            circuit.openUntil = System.currentTimeMillis() + properties.getCircuitOpenDuration().toMillis();
            log.warn("{} 连续失败达阈值 {}，熔断 {}ms", service, properties.getCircuitFailureThreshold(),
