...
/**
 * Agent Runtime 代理请求的统一构造器，收敛 IntentService / ChatSseService / AgentFlowService
 * 中重复的 messages、budget、metadata 组装逻辑，保证 profile、job_agent 等公共字段一致。
 */
public final class RuntimeRequestBuilder {
    ...
    private RuntimeRequestBuilder(String sessionId, String userMessage, String entrypoint) {
        ...
        metadata.put("profile", "job-agent");
        metadata.put("job_agent", true);
        metadata.put("entrypoint", entrypoint);
    }

    public static RuntimeRequestBuilder forEntrypoint(String sessionId, String userMessage, String entrypoint) {
    ...
    /**
     * 从 runtime 返回结果中提取 job_agent_directive，优先 tool_results 中的 directive 输出，
     * 其次顶层 directive 字段。返回可变副本，调用方可安全追加字段。
     */
    ...
    public static Map<String, Object> extractDirective(Map<String, Object> runtimeResult) {
        ...
        if (toolResults instanceof List) {
            for (Object item : (List) toolResults) {
                ...
                if (output instanceof Map && "job_agent_directive".equals(((Map) output).get("type"))) {
                    return new LinkedHashMap<String, Object>((Map<String, Object>) output);
                }
            }
        }
        Object directive = runtimeResult.get("directive");
        if (directive instanceof Map) return new LinkedHashMap<String, Object>((Map<String, Object>) directive);
