package com.jobagent.backend.modules.resume.storage;
...
public class ResumeObjectStorage {
    ...
    public ResumeObjectStorage(JobAgentProperties properties, MinioClient minioClient) {
    ...
    public Path downloadToTempFile(ResumeRecord record, String workspaceDir) {
        try {
            ...
            Path tempFile = dir == null
                    ? Files.createTempFile("job-agent-resume-", "." + record.getSuffix())
                    : Files.createTempFile(dir, "job-agent-resume-", "." + record.getSuffix());
