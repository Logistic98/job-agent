...
@pytest.mark.asyncio
async def test_job_agent_profile_routes_job_recommendation_in_runtime():
    executor = AgentExecutor(use_llm=False)
    request = AgentRunRequest(
        messages=[ChatMessage(role="user", content="帮我筛选上海大模型应用开发 40-50K 岗位")],
        metadata={"profile": "job-agent", "job_agent": True},
    )
    ...
    assert directive["type"] == "job_agent_directive"
...
@pytest.mark.asyncio
async def test_job_agent_profile_routes_more_jobs_with_previous_slots():
    executor = AgentExecutor(use_llm=False)
    request = AgentRunRequest(
...
@pytest.mark.asyncio
async def test_job_agent_profile_routes_resume_match_without_boss_search():
    executor = AgentExecutor(use_llm=False)
    request = AgentRunRequest(
        messages=[ChatMessage(role="user", content="分析当前简历是否匹配 Agent 应用开发岗位")],
        metadata={"profile": "job-agent", "job_agent": True, "current_jobs_count": 0},
    )
...
@pytest.mark.asyncio
async def test_job_agent_profile_routes_code_generation_to_complex_runtime_task():
    executor = AgentExecutor(use_llm=False)
    request = AgentRunRequest(
