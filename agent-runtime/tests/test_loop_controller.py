...
def _bff_profile() -> ProfileDefinition:
    return ProfileDefinition(
        id="job-agent",
        name="Job Agent",
        directive_type="job_agent_directive",
...
def test_budget_blocks_on_turns_tools_failures():
    ...
    assert controller.evaluate_budget({"turn_count": 99, "budget": {"max_turns": 5}}).stop_reason == StopReason.MAX_TURNS.value
    assert controller.evaluate_budget({"tool_call_count": 5, "budget": {"max_tool_calls": 5}}).stop_reason == StopReason.TOOL_BUDGET_EXCEEDED.value
    assert controller.evaluate_budget({"failure_count": 3, "budget": {"max_failures": 3}}).stop_reason == StopReason.MAX_FAILURES.value
...
def test_route_bff_directive_only_unless_runtime_entrypoint_and_planner():
    ...
    assert controller.route_after_task_understanding(
        {**base, "directive": {"next_action": "call_get_recommend_jobs"}, "metadata": {"entrypoint": "chat.ask"}}, profile
    ) == "finalize"
    ...
    assert controller.route_after_task_understanding(
        {**base, "directive": {"next_action": "run_runtime_planner"}, "metadata": {"entrypoint": "chat.stream"}}, profile
    ) == "finalize"
    # 要求 planner 且 entrypoint 在白名单 -> 进入 Loop
    assert controller.route_after_task_understanding(
        {**base, "directive": {"next_action": "run_runtime_planner"}, "metadata": {"entrypoint": "agent.run"}}, profile
    ) == "collect_context"
...
def test_route_after_plan_and_budget_and_reflect():
    ...
    assert controller.route_after_reflect({"turn_count": 1, "budget": {"max_turns": 5}}) == "tool_search"
