...
def test_config_loads_env_placeholders(tmp_path, monkeypatch):
    ...
    config_path.write_text(
        """
llm_service:
  provider: "${JOB_AGENT_LLM_PROVIDER:deepseek_api}"
  base_url: "${JOB_AGENT_LLM_BASE_URL:http://default.local/v1}"
  api_key: "${JOB_AGENT_LLM_API_KEY:}"
  timeout_seconds: "${JOB_AGENT_LLM_TIMEOUT_SECONDS:60}"
runtime:
  use_llm_planner: "${JOB_AGENT_RUNTIME_USE_LLM_PLANNER:false}"
""",
        encoding="utf-8",
    )
    monkeypatch.setenv("JOB_AGENT_LLM_BASE_URL", "https://example.com/v1/chat/completions")
    monkeypatch.setenv("JOB_AGENT_LLM_API_KEY", "test-secret")
    monkeypatch.setenv("JOB_AGENT_LLM_TIMEOUT_SECONDS", "15")
    monkeypatch.setenv("JOB_AGENT_LLM_PROVIDER", "chatgpt_pro")
    monkeypatch.setenv("JOB_AGENT_RUNTIME_USE_LLM_PLANNER", "false")
