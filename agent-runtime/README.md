...
基于 LangGraph 的智能体运行时核心实现，面向二期 ToolOps 工具体系设计。项目结构参考 `job-agent-engine` 的 Python 工程风格，并吸收 `claude-code-fork-main` 中工具自描述、权限前置、工具注册、Agent Loop、检查点和可观测的核心实现思想。
...
```shell
docker build -t job_agent_runtime:latest .
docker run --name job_agent_runtime -p 8010:8010 -d job_agent_runtime:latest
```

模型服务统一通过 [config/config.yaml](config/config.yaml) 声明配置结构，连接地址、密钥等敏感值通过环境变量注入；如需在容器中挂载外部配置文件：

```shell
docker run --name job_agent_runtime \
  -p 8010:8010 \
  -v $(pwd)/config/config.yaml:/app/config/config.yaml \
  -d job_agent_runtime:latest
```
...
```yaml
llm_service:
  base_url: "${JOB_AGENT_LLM_BASE_URL:https://api.deepseek.com/chat/completions}"
  api_key: "${JOB_AGENT_LLM_API_KEY:}"
  model_name: "${JOB_AGENT_LLM_MODEL_NAME:deepseek-v4-pro}"
  timeout_seconds: "${JOB_AGENT_LLM_TIMEOUT_SECONDS:60}"
  prompt_cache_enabled: "${JOB_AGENT_LLM_PROMPT_CACHE_ENABLED:true}"
  prompt_cache_strategy: "${JOB_AGENT_LLM_PROMPT_CACHE_STRATEGY:stable-prefix}"

tool_search:
  enabled: "${JOB_AGENT_TOOL_SEARCH_ENABLED:true}"
  limit: "${JOB_AGENT_TOOL_SEARCH_LIMIT:8}"
  fallback_limit: "${JOB_AGENT_TOOL_SEARCH_FALLBACK_LIMIT:5}"
```
...
```shell
export JOB_AGENT_CONFIG=/path/to/config.yaml
```
