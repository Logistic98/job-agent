...
class WebSearchConfig(BaseModel):
    ...
    bocha_ai_endpoint: str = "https://api.bochaai.com/v1/ai-search?utm_source=job-agent-runtime"
...
class RuntimeSettings(BaseModel):
    ...
    环境变量由 config.yaml 中的占位符解析,避免 JOB_AGENT_CONFIG 被 pydantic-settings
    ...
    def load(cls, config_path: str = None) -> "RuntimeSettings":
        ...
        if use_dotenv:
            _load_dotenv_files()
        path = config_path or os.getenv("JOB_AGENT_CONFIG") or _dotenv_values.get("JOB_AGENT_CONFIG") or "config/config.yaml"
