...
run_runtime_eval() {
  ...
  env -u JOB_AGENT_RUNTIME_USE_LLM_PLANNER uv run python -m pytest -q \
    tests/test_job_agent_router.py \
...
run_intent_eval() {
  ...
  env -u JOB_AGENT_RUNTIME_USE_LLM_PLANNER uv run python -m pytest -q tests/test_job_domain.py tests/test_service.py || fail "agent-intent: regression failed"
  popd >/dev/null
}
