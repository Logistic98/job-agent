...
`job-agent` 是一个面向求职场景的本地 Agent 工作台。当前项目已经从早期的通用 Agent 平台雏形，演进为“Vue 前端 + Spring Boot 业务后端 + Python Agent Runtime + Runtime 按需工具 + 评估/记忆/工具/沙箱辅助服务”的多服务工程。
...
- `JOB_AGENT_LLM_*`：Runtime 的 OpenAI 兼容模型配置。
- `JOB_AGENT_MINIO_*`：简历附件和资源对象存储配置。
